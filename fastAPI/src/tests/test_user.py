...
def test_user_log_in(test_client, mocker):
    ...
    verified = mocker.patch.object(UserService, "verify_password", return_value=True)
    access_token = mocker.patch.object(
        UserService, "create_jwt", return_value="access_token"
    )
    ...
    # 401
    verified = mocker.patch.object(UserService, "verify_password", return_value=False)
    body = {"username": "admin", "password": "None"}
    response = test_client.post("/users/log-in", json=body)
    assert response.status_code == 401
    assert response.json() == {"detail": "Not Authorized"}

    # 404
    mocker.patch.object(UserRepository, "get_user_by_username", return_value=None)
